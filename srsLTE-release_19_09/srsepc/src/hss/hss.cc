...
#if(NUK_UE && NUK_EPC)
bool hss::write_nuk_db_file(std::string db_filename)
{
  std::string line;
  uint8_t     k[16];
  uint8_t     amf[2];
  ...
  m_db_file << "#                                                                                           \n"
            ...
            << "#                                                                                           \n"
            << "# Note: Lines starting by '#' are ignored and will be overwritten by NUK                    \n";
  ...
  uint8_t nukcount = 1;
  std::string nukcount_s = std::to_string(nukcount);
  
  m_db_file << "Get_ue";
  m_db_file << nukcount_s;
  m_db_file << ",";
  m_db_file << "xor";                                 // xor or milenage
  m_db_file << ",";
  m_db_file << nuk_imsi;                              // imsi
  m_db_file << ",";
  m_db_file << "00000000000000000000000000000000";    //K
  m_db_file << ",";
  m_db_file << "opc";                                 // OP or OPC
  m_db_file << ",";
  m_db_file << "00000000000000000000000000000000";    // OP or OPC code
  m_db_file << ",";
  m_db_file << "0000";    							  // AMF
  m_db_file << ",";
  m_db_file << "000000000000";                        // SQN
  m_db_file << ",";    
  m_db_file << "7";                                   // QCI
  m_db_file << ",dynamic";
  m_db_file << std::endl;
...
bool hss::read_nuk_db_file(std::string db_filename)
{
	std::ifstream m_db_file;
...
{ 
	...
	if (ret == false) {
	  ...
	  m_hss_log->console("Writed User IMSI in HSS. IMSI: %015" PRIu64 "\n", nuk_imsi);
	  write_nuk_db_file(db_file);
	}
	
	m_hss_log->console("HSS Re-Initialized.\n");
	read_nuk_db_file(db_file);

	ret = get_ue_ctx(imsi, &ue_ctx);
