                                 ...
                                 srslte::log*            nas_log)
{
  ...
  if (imsi == 0) {
    nas_log->console("0 -- Could not find IMSI from M-TMSI. M-TMSI 0x%x\n", m_tmsi);
    nas_log->error("0 -- Could not find IMSI from M-TMSI. M-TMSI 0x%x\n", m_tmsi);
                                ...
                                srslte::log*            nas_log)
{
  ...
  uint64_t imsi = s1ap->find_imsi_from_m_tmsi(m_tmsi);
  if (imsi == 0) {
    nas_log->console("1 -- Could not find IMSI from M-TMSI. M-TMSI 0x%x\n", m_tmsi);
    nas_log->error("1 -- Could not find IMSI from M-TMSI. M-TMSI 0x%x\n", m_tmsi);
    return true;
  }
                                              ...
                                              srslte::log*            nas_log)
{
  ...
  //rewrited by NUK start
  
  /*uint64_t imsi = s1ap->find_imsi_from_m_tmsi(m_tmsi);
  if (imsi == 0) {
    nas_log->console("2 -- Could not find IMSI from M-TMSI. M-TMSI 0x%x\n", m_tmsi);
    nas_log->error("2 -- Could not find IMSI from M-TMSI. M-TMSI 0x%x\n", m_tmsi);
    return true;
  }
  ...
  nas_log->console("Packing --- Tracking area update reject.\n");
...
bool nas::handle_identity_response(srslte::byte_buffer_t* nas_rx)
{
  ...
  //rewrited by NUK start 20201207
