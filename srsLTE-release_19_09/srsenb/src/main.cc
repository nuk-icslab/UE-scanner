...
void parse_args(all_args_t *args, int argc, char* argv[]) {
  ...
  common.add_options()
	...
	#if(NUK_UE && NUK_eNB)
	("log.NUK_level",     bpo::value<string>(&args->log.NUK_level)->default_value("info"),   "NUK log level")
    ("log.NUK_hex_limit", bpo::value<int>(&args->log.NUK_hex_limit)->default_value(32),  "NUK log hex dump limit")
	#else
  ...
  #if(NUK_UE && NUK_eNB)
  if (vm.count("log.NUK_level")) {
	  if(!vm.count("log.phy_level")) {
		  args->phy.log.phy_level = args->log.NUK_level;
	  }
	  if (!vm.count("log.phy_lib_level")) {
		  args->phy.log.phy_lib_level = args->log.NUK_level;
	  }
	  if(!vm.count("log.mac_level")) {
		  args->stack.log.mac_level = args->log.NUK_level;
	  }
	  if(!vm.count("log.rlc_level")) {
		  args->stack.log.rlc_level = args->log.NUK_level;
	  }
	  if(!vm.count("log.pdcp_level")) {
		  args->stack.log.pdcp_level = args->log.NUK_level;
	  }
  }
  #else
  ...
  #if(NUK_UE && NUK_eNB)
  if (vm.count("log.NUK_hex_limit")) {
	  if(!vm.count("log.phy_hex_limit")) {
		  args->log.phy_hex_limit = args->log.NUK_hex_limit;
	  }
	  if(!vm.count("log.mac_hex_limit")) {
		  args->stack.log.mac_hex_limit = args->log.NUK_hex_limit;
	  }
	  if(!vm.count("log.rlc_hex_limit")) {
		  args->stack.log.rlc_hex_limit = args->log.NUK_hex_limit;
	  }
	  if(!vm.count("log.pdcp_hex_limit")) {
		  args->stack.log.pdcp_hex_limit = args->log.NUK_hex_limit;
	  }
  }
  #else
